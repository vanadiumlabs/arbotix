...
class BioloidController
{
  public:
    ...
    unsigned char * id;                         // servo id for this index
    
  private:  
    unsigned int * pose;                        // the current pose, updated by Step(), set out by Sync()
    unsigned int * nextpose;                    // the destination pose, where we put on load
    int * speed;                                // speeds for interpolation 
